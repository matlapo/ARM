...
enable_PB_INT_ASM:
			LDR R2, =PB_BASE	//Load address of pb base
			AND R1, R0, #0xF	//Only load bits we want to enable
			STR R1, [R2, #0x8]	//Store it back into the register
			BX LR

disable_PB_INT_ASM:
			LDR R2, =PB_BASE	//Load address of pb base
			LDR R1, [R2, #0x8]	//Load value at interrupt mask
			BIC R1, R1, R0		//Bit clear the corresponding bit(s)
			STR R1, [R2, #0x8]	//Store back into the register
			BX LR
